...
public class AppSettings
{
    ...
    private void SeedMcpResources()
    {
        ...
        var defaults = new List<McpResourceConfig>
        {
            ...
            new() { Name = "天上粮仓·话语广场", ResourceType = "HttpMcp",
                     Path = "https://infohub.busanblog.com/api/mcp",
                     Description = "每日灵粮MCP服务, 提供优质话语段落和文章" },
    ...
    private void SeedPromptTemplates()
    {
        ...
        var articlePrompt = new PromptTemplate
        {
            ...
            UserPromptTemplate = @"你是一位信仰灵修类公众号的专业创作者。即将为你提供一段原文素材（可能是生活记录、日记片段、梦境、日常琐事等）。

【核心任务】原文素材仅供灵感参考——你必须创作一篇全新的文章，而不是改写或润色原文。

【严禁行为】
- 禁止保留原文中的任何具体事件（如打球、吃饭、上班、做梦等）
- 禁止出现原文中的任何人名、地名、时间
- 禁止原文的叙事结构、事件顺序
- 禁止以今天/昨天/最近开头——这是文章，不是日记

【正确做法】
1. 只从原文中提取【灵性感悟的内核】：如感恩的心、愤怒后的饶恕、等候中的盼望、平凡中的恩典
2. 用这些内核创作全新的场景、全新的故事、全新的人物对话
3. 文章的结构、人物、事件，全部由你重新创造
4. 把灵修感悟编织成一个完整的新故事——开头吸引、中间层层递进、结尾「愿...」收尾
5. 适度引用合和本经文，与文章主题自然呼应

【示例对比】
❌ 错误做法: 今天和朋友打了篮球，感受到生活的美好。（保留原文事件，只是换表达）
✅ 正确做法: 黄昏时分，他站在空旷的球场上。篮球撞击地面的节奏，像心跳，也像生命中那些重复的等待...（保留感恩的内核，但场景、人物、事件全部重新创造）

【输出格式】
- 标题（灵修类，吸引人，不要太长）
- 简介（1-2句话，点出灵修主题，引人阅读）
- 经文（放在最前，合集本，与文章核心呼应）
- 正文内容（结构清晰的灵修文章，自然分段，不用小标题）
- 5个两字标签（共10个字，如：恩典 信心 盼望 感恩 生命）

【输出格式】
- 标题（灵修类，吸引人，不要太长）
- 简介（1-2句话，点出灵修主题，引人阅读）
- 经文（放在最前，与文章核心呼应，使用和合本）
- 正文内容（结构清晰的灵修文章，自然分段，不用小标题）
- 5个两字标签（共10个字，如：恩典 信心 盼望 感恩 生命）
